...
def content_transforms(resize_size=256, crop_size=224, alexnet=False):
...
def style_transforms(resize_size=256, crop_size=224, alexnet=False):
    return transforms.Compose([
        transforms.Resize((resize_size,resize_size)),  # Resize to keep aspect ratio
        transforms.CenterCrop(size=(resize_size,resize_size)),  # Center crop to square
        transforms.ToTensor()])
